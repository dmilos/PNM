  ...
  namespace _internal
   {
    ...
    inline bool load_NL( std::istream& is )
     {
      auto begin = is.tellg();
      auto ch = is.get();
      if( ( ch != 0x0d ) && ( ch != 0x0a ) )
    ...
    inline bool load_space( std::istream& is )
     {
        ...
        bool process( std::istream& is )
         {
          ...
          if( false == PNM::_internal::load_blank( is ) ){ is.seekg( begin ); return false; }
